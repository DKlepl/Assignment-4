...
#delete 19th row
all_param_Resp=all_param_Resp[-19,]
...
param_all_HR = list(
  ...
  radius=median(all_param_HR$radius)+1.2
)
param_all_HR
...
param_all_Resp = list(
  ...
  radius=median(all_param_Resp$radius)+1.2
)
param_all_Resp
```
```{r run CRQA}
...
get_rqa_HR= function (df) {
  ...
  LAM_HR=results$LAM
  TT_HR = results$TT
  rqa_df = data.frame(RR_HR,DET_HR,maxL_HR,L_HR,ENTR_HR,LAM_HR,TT_HR)
  return(rqa_df)
}
...
get_rqa_Resp= function (df) {
  ...
  TT_Resp = results$TT
  rqa_df = data.frame(RR_Resp,DET_Resp,maxL_Resp,L_Resp,ENTR_Resp,LAM_Resp,TT_Resp)
  return(rqa_df)
}
...
```{r shuffle}
test=read.csv(file="PreprocessedData/Study1_G3_T2_TurnTaking.csv",header=T)
test$Resp1S_shuffled=sample(test$Resp1S)
...
write.csv(final_results_shuffled,"final_results_shuffled",row.names=F)
```
...
summary(RR_Resp_shuf)

#other indeces are 0 or NA therefore it doesnt make sense to compare them
```
...
for(cond in conditions) {
  ...
  for (g in 1:nrow(combinations)) {
    ...
    file1 = study1_flist[grepl(file1,study1_flist) & grepl(cond,study1_flist)]
    file2 = study1_flist[grepl(file2,study1_flist) & grepl(cond,study1_flist)]
...
for(cond in conditions) {
  ...
  for (g in 1:nrow(combinations)) {
    ...
    #from list of files extract those that match file1&2 +condition
    file1 = study2_flist[grepl(file1,study2_flist) & grepl(cond,study2_flist)]
    file2 = study2_flist[grepl(file2,study2_flist) & grepl(cond,study2_flist)]
