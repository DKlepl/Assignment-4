...
normal_surrogate$condition = relevel(normal_surrogate$condition,ref="TurnTaking")
...
summary(maxL_HR_sur) #yes, TurnTaking x SelfPaced
...
#exception is TurnTaking for maxL which however doesnt make much sense because there we didnt expect any coordination except for PC determined pace and turn taking
```
...
### Testing effects of conditions
 ...
 I'm including surrogate pairs (using merged dataframe) and type as random effect (no systematic difference was found). Because there is no difference between real pairs and surrogate (in no condition) I can use all these data to increase power of the analysis.
 
```{r effect of condition}
#make TurnTaking baseline, where we expect coordination only because of computer determined rhythm
final_results$condition = relevel(final_results$condition,ref="TurnTaking")
...
summary(model_maxL_HR) #everything except Conversation, SelfPaced has biggest difference

model_LAM_HR = lmer(LAM_HR ~ condition+(1|group)+(1|study), data=final_results,REML=F)
summary(model_LAM_HR) #SelfPaced
